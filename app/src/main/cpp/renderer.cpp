...
bool Renderer::init(ANativeWindow* window) {
    if (mInitialized) {
        LOGI("Renderer::init déjà fait, skip");
        return true;
    }
    if (!window) { LOGE("Renderer::init window null"); return false; }

    mDisplay = eglGetDisplay(EGL_DEFAULT_DISPLAY);
    if (mDisplay == EGL_NO_DISPLAY) { LOGE("eglGetDisplay failed"); return false; }
    if (!eglInitialize(mDisplay, nullptr, nullptr)) { LOGE("eglInitialize failed"); return false; }
    ...
    if (!eglChooseConfig(mDisplay, attribs, &config, 1, &numConfigs) || numConfigs < 1) {
        LOGE("eglChooseConfig failed (%d configs)", numConfigs); return false;
    }

    // Important sur certains GPU : aligner le format de la window sur celui d'EGL.
    EGLint format = 0;
    ...
    if (mSurface == EGL_NO_SURFACE) { LOGE("eglCreateWindowSurface failed: 0x%x", eglGetError()); return false; }

    const EGLint ctxAttribs[] = { EGL_CONTEXT_CLIENT_VERSION, 3, EGL_NONE };
    mContext = eglCreateContext(mDisplay, config, EGL_NO_CONTEXT, ctxAttribs);
    if (mContext == EGL_NO_CONTEXT) { LOGE("eglCreateContext failed: 0x%x", eglGetError()); return false; }
    ...
    LOGI("Renderer initialisé, taille %dx%d", mWidth, mHeight);
    mInitialized = true;
    return true;
}
